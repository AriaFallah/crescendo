/*
 * This file provides a simple interface to read images from the
 * Hackathon demo board on Windows. It will work on other platforms
 * with a little modification (removing _kbhit() and _getch() calls
 * from main()). To build, type "nmake" from a Visual Studio 64-bit
 * command prompt. You must have the FTDI D2XX driver installed.
 *
 ...
 *
 * Because printing the images is so slow, this does not run at the
 * full 80-Hz frame rate. Change the SKIP_FRAMES variable defined
 * below to set the frame rate you want to use.
 */
...
#define SKIP_FRAMES 50
    ...
    c = _getch();
    if (c == ' ') baselineTaken = 0; else break;
  }
  sensorClose();
}
